...
app.post("/billing/create-checkout-session", authRequired, async (req, res) => {
  try {
    ...
    console.error(e); res.status(500).json({ error: "stripe failed" });
  }
});
...
async function setSubscription(userId, status, provider='stripe', external_id=null) {
  ...
  } catch (e) { console.error('setSubscription failed', e); }
}
...
app.post("/api/generateTherapy", generateLimiter, authOptional, async (req, res) => {
  try {
    ...
    if (WEBHOOK_URL) { try { await fetch(WEBHOOK_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ event:'therapy.created', id, user_id:req.user?.id||null, created_at:new Date().toISOString() }) }); } catch(e){ console.error('webhook failed', e); } }
...
async function retentionSweep(){
  ...
  try {
    ...
    console.log("Retention sweep done");
...
} catch(e) { console.error('SQLite alter users failed', e); }
