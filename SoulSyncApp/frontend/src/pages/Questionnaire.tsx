    ...
    <div className="p-4 max-w-2xl mx-auto">
      ...
      <div className="space-y-3 mb-4">
        <label className="block">
          ...
          <input value={disease} onChange={e=>setDisease(e.target.value)} className="w-full border dark:border-gray-700 rounded px-3 py-2 bg-transparent" placeholder={lang==='en'?'e.g., Depression':'npr. Depresija'} />
        </label>
        <label className="block">
          <span className="block text-sm opacity-80">{lang==='en'?'Symptoms (comma‑separated)':'Simptomi (odvojeni zarezom)'}</span>
          <input value={symptoms} onChange={e=>setSymptoms(e.target.value)} className="w-full border dark:border-gray-700 rounded px-3 py-2 bg-transparent" placeholder={lang==='en'?'fatigue, anxiety, insomnia':'umor, anksioznost, nesanica'} />
        </label>
      ...
      {therapy && (
        <div className="space-y-3">
          ...
          {pdfUrl && (
            <a className="inline-block mt-2 px-4 py-2 rounded border dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-800" href={pdfUrl} target="_blank" rel="noreferrer">
